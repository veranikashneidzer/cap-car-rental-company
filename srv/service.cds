...
service CarRentalCompanyService {
    ...
    entity Cars as projection on carRentalCompany.Cars {
        *,
        (
            case
                when exists rentals[rentalDate < $now and returnDate > $now] then 'STATUS_2'
                when exists maintenance[startDate < $now and endDate > $now] then 'STATUS_1'
                else 'STATUS_3'
            end
        ) as status : CarStatuses:code,
        statusData : Association to CarStatuses on statusData.code = status,
    };
